...
use fks_audit::formatter::{CodeFormatter, FormatMode, Formatter};
use fks_audit::grok_reasoning::{FileForAnalysis, GrokReasoningClient};
use fks_audit::llm::LlmClient;
use fks_audit::prelude::*;
use fks_audit::scoring::FileScorer;
use fks_audit::tree_state::{FileCategory, TreeStateManager};
use fks_audit::{
...
fn run_tree_visualization(
    ...
    use fks_audit::directory_tree::DirectoryTreeBuilder;
...
async fn run_llm_audit(
    ...
    use fks_audit::llm_audit::{AuditMode, LlmAuditor};
...
async fn run_grok_audit(
    ...
    use fks_audit::cache::AuditCache;
    use fks_audit::grok_reasoning::{analyze_all_batches, RetryConfig};
    use fks_audit::llm_config::{CacheConfig, LlmConfig};
    ...
    let files_to_analyze: Vec<&fks_audit::tree_state::FileState> = if changed_only {
        if let Some(previous_state) = manager.load_previous_state()? {
            let diff = manager.diff(&previous_state, &current_state);
    ...
    match llm_config.check_budget(estimated_cost) {
        fks_audit::llm_config::BudgetStatus::Warning {
            current,
            limit,
            usage_pct,
        } => {
        ...
        fks_audit::llm_config::BudgetStatus::Exceeded { current, limit } => {
            println!(
...
async fn run_janus_audit(
    ...
    use fks_audit::llm_audit::LlmAuditor;
    use fks_audit::llm_config::claude_models;
