...
async fn get_cache_entries(cache: &RepoCacheSql) -> Vec<CacheRow> {
    let rows = sqlx::query_as::<_, (String, String, Option<i64>, i64, String, String, i64, String, String, Vec<u8>)>(
...
async fn get_cache_overview(cache: &RepoCacheSql) -> CacheOverviewStats {
    let (total_entries, total_tokens, total_file_size) = sqlx::query_as::<_, (i64, Option<i64>, i64)>(
    ...
    let unique_files = sqlx::query_scalar::<_, i64>(
        "SELECT COUNT(DISTINCT file_path) FROM cache_entries"
    )
    .fetch_one(&cache.pool)
    .await
    .unwrap_or(0);
    ...
    // Approximate with blended $10/1M
    let estimated_cost = total_tokens as f64 * 10.0 / 1_000_000.0;
...
pub async fn cache_overview_handler(
    State(state): State<Arc<WebAppState>>,
) -> impl IntoResponse {
    let pool = &state.db.pool;

    // Get all repositories
    let repos: Vec<(String, String, String, Option<String>)> = sqlx::query_as(
        "SELECT id, name, path, git_url FROM repositories ORDER BY name"
    )
    .fetch_all(pool)
    .await
    .unwrap_or_default();
    ...
    for (repo_id, repo_name, repo_path, _git_url) in &repos {
        if let Some(cache) = open_repo_cache(repo_path).await {
            ...
            let coverage_pct = if stats.total_entries > 0 { "partial" } else { "none" };
...
pub async fn cache_repo_detail_handler(
    ...
    // Get repo info
    let repo: Option<(String, String)> = sqlx::query_as(
        "SELECT name, path FROM repositories WHERE id = ?"
    )
    .bind(&repo_id)
    .fetch_optional(pool)
    .await
    ...
    let cache = match open_repo_cache(&repo_path).await {
        ...
        None => return Html(format!(
            ...
            style = page_style(), nav = nav("Cache Viewer"), name = html_escape(&repo_name)
        )),
    };
...
pub async fn cache_file_detail_handler(
    ...
    let entry: Option<(String, Option<i64>, i64, String, String, i64, String, String, Vec<u8>)> =
        sqlx::query_as(
    ...
    let (cache_type, tokens, file_size, created, last_accessed, access_count, provider, model, blob) =
        match entry {
            Some(e) => e,
            None => {
                return Html(format!(
    ...
    Html(format!(
        ...
        tokens = tokens.map(|t| format_tokens(t)).unwrap_or_else(|| "—".to_string()),
...
pub async fn cache_gaps_handler(
    ...
    let analyzable_extensions = ["rs", "py", "js", "ts", "tsx", "sh", "kt", "java", "go", "rb"];
    let skip_dirs = ["dist", "build", "node_modules", "target", ".git", "vendor",
    ...
    if repo_root.exists() {
        ...
        walk_dir(repo_root, repo_root, &mut all_source_files, &analyzable_extensions, &skip_dirs);
    }
    ...
    for f in &pending_files {
        let dir = f.rsplit_once('/').map(|(d, _)| d.to_string()).unwrap_or_else(|| ".".to_string());
    ...
    // Coverage bar color
    let bar_color = if coverage_pct >= 80.0 { "#22c55e" }
        else if coverage_pct >= 50.0 { "#f59e0b" }
        else { "#ef4444" };
