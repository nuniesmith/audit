...
use crate::db::task::{Task, TaskGroup};
...
pub fn group_by_file(tasks: Vec<Task>) -> Vec<TaskGroup> {
    ...
    for task in tasks {
        let key = task.source_file.clone()
...
pub fn group_by_repo(tasks: Vec<Task>) -> Vec<TaskGroup> {
    ...
    for task in tasks {
        let key = task.source_repo.clone()
...
pub fn filter_ready_groups(groups: Vec<TaskGroup>) -> Vec<TaskGroup> {
    groups
        .into_iter()
        .map(|mut g| {
            g.tasks.retain(|t| t.status == "ready" || t.status == "review");
            g
        })
...
pub fn tasks_are_similar(task1: &Task, task2: &Task) -> bool {
    ...
    if task1.category == task2.category && task1.source_repo == task2.source_repo {
        ...
        let words1: std::collections::HashSet<_> = task1.content
            .to_lowercase()
            .split_whitespace()
            .filter(|w| w.len() > 3)
            .collect();
        
        let words2: std::collections::HashSet<_> = task2.content
            .to_lowercase()
            .split_whitespace()
            .filter(|w| w.len() > 3)
            .collect();
...
pub fn merge_groups(group1: TaskGroup, group2: TaskGroup) -> TaskGroup {
    let combined_tasks: Vec<Task> = group1.tasks
        .into_iter()
...
mod tests {
    ...
    fn test_smart_grouping() {
        let tasks = vec![
            make_task("Refactor function A", Some("src/main.rs"), Some("refactor"), 5),
            make_task("Refactor function B", Some("src/main.rs"), Some("refactor"), 6),
