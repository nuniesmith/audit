...
impl Aggregator {
    ...
    pub async fn aggregate(
        ...
        let sections: Vec<ReportSection> = successful.iter()
            .map(|r| ReportSection {
                title: r.subtopic.clone(),
                content: r.findings.clone(),
                sources: r.sources.as_ref()
        ...
        let (summary, key_findings, recommendations) = self
            .synthesize(request, &sections)
            .await?;
        
        let total_tokens: i64 = results.iter().map(|r| r.tokens_used).sum();
        let avg_confidence = successful.iter().map(|r| r.confidence).sum::<i32>() 
            / successful.len() as i32;
    ...
    async fn synthesize(
        ...
        let sections_text: String = sections.iter()
            .map(|s| format!("## {}\n\n{}", s.title, s.content))
            .collect::<Vec<_>>()
            .join("\n\n---\n\n");
...
impl ResearchReport {
    ...
    pub fn to_markdown(&self) -> String {
        ...
        md.push_str(&format!("**Workers:** {}/{} succeeded | ", 
    ...
    pub fn to_zed_format(&self) -> String {
        ...
        output.push_str(&format!("Confidence: {}/10 | {} workers\n\n", 
